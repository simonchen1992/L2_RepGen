...
def fill_in_data(ics_dic, tem, output_path):
    ...
    for formCell in sheet_template['E']:
        ...
        if formID is not None:
            for seek in ics_dic.keys():
                if seek == formID:
                    output = ics_dic[seek]
                    #  the output type of pdf is unicode/string, need to be took into consider
                    if formID == 'JlX2RldmljZV9WQ1BTMi4yLmh0bWwA.section5.irwinspec':
                        output = 'Yes' if output in [1.1, '1.1'] else 'No'
                    if formID == 'JlX2RldmljZV9WQ1BTMi4yLmh0bWwA.section7.maxdrlsets' and output not in [None, '']:
                        output = 'Yes' if int(output) > 4 else 'No'
        ...
        if ref.value not in [None, 'Reference']:
            ...
            if tem == 'Template_22.xlsx' and sheet_template['C' + str(ref.row)].value == 3.14:
                if sheet_template['G' + str(ref.row)].value == 'No' and sheet_template['G' + str(ref.row + 1)].value == 'Yes':
                    sheet_template['G' + str(ref.row)].fill = style
    template.save(output_path + '_output.xlsx')
    template.close()
    print 'Fill in data succeeded!\n'

#  compare output template with manual one
def compare(exl_a, exl_b):
    dic_a = {}
    dic_b = {}
    diff = []
    try:
        file_a = load_workbook(exl_a)
...
def main():
    action = raw_input('------ What do you want to do with checktool ------\n'
    ...
    if action == '1':
        ...
        fill_in_data(ics_dic, temp, path)
    if action == '2':
        file_a = raw_input('------ Please input the path of comparision Excel_A. ------\n'
                           'Note: Please remove "Data Validation Format" before comparing the files!\n')
        file_b = raw_input('------ Please input the path of comparision Excel_B. ------\n'
                           'Note: Please remove "Data Validation Format" before comparing the files!\n')
...
def test():
    ...
    # print ics_dic1
    # print ics_dic2
    # #fill_in_data(ics_dic1, 'template.xlsx', 'out1.pdf')
